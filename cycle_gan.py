...
class cycle_gan():
    ...
    def build_model(self):
        ...
        with tf.compat.v1.variable_scope('optimizer') as scope:
            self.train_discriminator_X_op = tf.compat.v1.train.AdamOptimizer(learning_rate=1e-4, beta1=0.5, beta2=0.9).minimize(
                self.discriminator_X_loss, 
                var_list = self.discriminator_X_variables
            )

            self.train_discriminator_Y_op = tf.compat.v1.train.AdamOptimizer(learning_rate=1e-4, beta1=0.5, beta2=0.9).minimize(
